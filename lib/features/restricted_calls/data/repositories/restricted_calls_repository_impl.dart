...
class RestrictedCallsRepositoryImpl
    implements RestrictedCallsRepository {
  static const String _restrictedContactsKey =
      'restricted_contacts';

  static const String _rejectedCallsKey =
      'rejected_calls';

  final SharedPreferences preferences;

  RestrictedCallsRepositoryImpl(
      this.preferences,
      );
  ...
  Future<List<RestrictedContact>> getRestrictedContacts() async {
    final values =
        preferences.getStringList(_restrictedContactsKey) ?? [];
  ...
  @override
  Future<void> addRestrictedContact(
  ...
  @override
  Future<void> removeRestrictedContact(
      String phoneNumber,
      ) async {
    final contacts = await getRestrictedContacts();
  ...
  Future<List<RejectedCall>> getRejectedCalls() async {
    final values =
        preferences.getStringList(_rejectedCallsKey) ?? [];

    return values
        .map(
          (value) => RejectedCall.fromJson(
        jsonDecode(value) as Map<String, dynamic>,
      ),
    )
        .toList();
